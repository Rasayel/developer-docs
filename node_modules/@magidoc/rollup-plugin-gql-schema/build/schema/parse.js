...
async function parseGraphqlSchema(options) {
...
async function readFullSchema(globPaths) {
    const paths = new Set((await Promise.all(globPaths.map((path) => readGlobPaths(path)))).flatMap((it) => it));
    if (paths.size === 0) {
        throw new Error(`No paths found matching target glob patterns: [${globPaths.toString()}].\nIf you used relative paths, make sure the paths are relative to where the node command was launched or use absolute paths.`);
    }
    return (await Promise.all(Array.from(paths.values()).map((path) => readFile(path).then((buff) => buff.toString())))).join('\n\n');
}
